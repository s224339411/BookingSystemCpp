...
#include "ui_dialogbookinglistweek.h"

DialogBookingListWeek::DialogBookingListWeek(QWidget *parent)
    : QDialog(parent)
    , ui(new Ui::DialogBookingListWeek)
{
    ui->setupUi(this);
    ...
    for (int i = 0; i < 7; ++i) {
        QDate current_day = monday.addDays(i);
        days_of_week.push_back(current_day);
    }
    ...
    for (int i = 0; i < days_of_week.size(); ++i) {
        add_table_row(ui->tableBooings,days_of_week[i]);
        for (int j = 0; j < bookings.size(); ++j) {
            if(days_of_week[i] == bookings[j].check_in_date){
                add_table_row(ui->tableBooings,
...
DialogBookingListWeek::~DialogBookingListWeek()
{
    delete ui;
}

void DialogBookingListWeek::add_table_row(QTableWidget *table,QDate &date){
    int row = table->rowCount();
    table->insertRow(row);

    table->setSpan(row, 0, 1, 8); // Merge 2 rows x 3 columns starting at (1,1)
    table->setItem(row, 0, new QTableWidgetItem(date.toString("dddd")));
}

void DialogBookingListWeek::add_table_row(QTableWidget *table,QDate &check_in_date,QDate &check_out_date,QString guest_name, QString guest_number, QString room,QString daily_cost,QString total_cost){
    int row = table->rowCount();
    table->insertRow(row);
